...
const timer = {
    tempTimer: 0,
    speed: 1000, //скорость змейки

    checkEatedFood() { //увеличиваем скорость, каждые 10 яблок
        if(food.eatedCount != 0 && food.eatedCount % 10 == 0) {
            this.speed -=100;
        }
    },
    
    start(event, speed = timer.speed) { // запускаем бесконечное движение
        timer.checkEatedFood();
 ...
 */
const food = {
    eatedCount: 0, //счётчик съеденной еды
    /**
    ...
    setEated() { // прибавляет количество съеденной еды
        this.eatedCount++;        
    },
    /**
